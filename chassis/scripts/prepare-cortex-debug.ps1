...
function Clear-CortexDebugState {
    ...
    foreach ($stateFile in $stateFiles) {
        ...
        $shouldRemove = $Aggressive.IsPresent
        $item = Get-Item -LiteralPath $stateFile

        if ($item.Length -eq 0) {
            $shouldRemove = $true
        }
        elseif (-not $Aggressive) {
            try {
    ...
    if ($removedFiles) {
        if ($Aggressive) {
            Write-Info "Reset persisted Cortex-Debug state files: $($removedFiles -join ', ')."
        }
        else {
...
try {
    ...
    Clear-CortexDebugState -Aggressive:$AggressiveStateReset
